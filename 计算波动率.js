...
async function addVolatilityWithStyle(inputFilePath) {
  ...
  for (const worksheet of workbook.worksheets) {
    ...
    for (const [year, list] of Object.entries(yearlyData)) {
      ...
      const tradingDays = prices.length; // 实际交易日数
      ...
      // 写入波动率值（只写这一列）
      worksheet.getCell(lastRowIdx, VOLATILITY_COL).value =
  ...
  const outputFilePath = path.join(dir, `${baseName}_波动率.xlsx`);
  await workbook.xlsx.writeFile(outputFilePath);
  console.log(`✅ 成功生成: ${outputFilePath}`);
}

// 使用
const inputPath = "./中证A50-中证A100-中证A500.xlsx";
